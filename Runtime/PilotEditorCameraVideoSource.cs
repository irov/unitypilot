...
{
    /// <summary>
    /// Editor-only video source. Captures the URP base camera into a RenderTexture and
    /// streams it via LiveKit's ScreenVideoSource pipeline, avoiding Device Simulator chrome.
    ///
    /// Uses URP's UniversalRenderPipeline.RenderSingleCamera (via reflection) when URP is
    /// active — Camera.Render() is unreliable / no-op under SRP.
    /// </summary>
    internal sealed class PilotEditorCameraVideoSource : ScreenVideoSource
    {
        ...
        static PilotEditorCameraVideoSource()
        {
            ...
            if (urpType != null)
            {
                s_renderSingleCameraMethod = urpType.GetMethod(
                    "RenderSingleCamera",
        ...
        {
            // Heartbeat: log every ~60 attempts so we can see whether the loop is alive.
            ...
            try
            {
                ...
                if (m_readRT == null || m_readRT.width != w || m_readRT.height != h)
                {
                    ...
                    _captureBuffer = new NativeArray<byte>(
                        w * h * GetStrideForBuffer(_bufferType),
                    ...
                    PilotLog.Info("PilotEditorCameraVideoSource: created RTs " + w + "x" + h
                ...
                // Render the camera into m_cameraRT.
                // Camera.Render() is unreliable under URP — prefer RenderSingleCamera if available.
...
#pragma warning restore CS0618
                ...
                AsyncGPUReadback.RequestIntoNativeArray(
                    ref _captureBuffer, m_readRT, 0, m_textureFormat, OnReadbackTracked);
...
namespace Pilot.SDK
{
    ...
    internal sealed class PilotEditorCameraVideoSource : ScreenVideoSource
    {
        ...
        static PilotEditorCameraVideoSource()
        {
            ...
            if (urpType != null)
            {
                // public static void RenderSingleCamera(ScriptableRenderContext, Camera)
                s_renderSingleCameraMethod = urpType.GetMethod(
                    "RenderSingleCamera",
                    BindingFlags.Public | BindingFlags.Static,
                    null,
        ...
        {
            PilotLog.Info("PilotEditorCameraVideoSource.Stop frames=" + m_frameCounter
                + " ok=" + m_readbackOkCounter + " err=" + m_readbackErrCounter);
        ...
        protected override bool ReadBuffer()
        {
            // Periodic log so we can see whether the loop is actually running.
            if (m_frameCounter - m_lastLoggedFrame >= 60)
            {
                m_lastLoggedFrame = m_frameCounter;
                PilotLog.Info("PilotEditorCameraVideoSource heartbeat: frame=" + m_frameCounter
            ...
            try
            {
                ...
                if (m_readRT == null || m_readRT.width != w || m_readRT.height != h)
                {
                    ...
                    PilotLog.Info("PilotEditorCameraVideoSource: created RT " + w + "x" + h
                        + " format=" + m_textureFormat);
                }
                ...
                // Render camera into m_cameraRT.
                // For URP, Camera.Render() is unreliable — use UniversalRenderPipeline.RenderSingleCamera.
                var prevTarget = cam.targetTexture;
                cam.targetTexture = m_cameraRT;

                if (s_renderSingleCameraMethod != null)
                {
                    // URP path. Suppress obsolete-API warnings from URP internals.
#pragma warning disable CS0618
                    s_renderSingleCameraMethod.Invoke(null, new object[] { default(ScriptableRenderContext), cam });
#pragma warning restore CS0618
                }
                ...
                // Y-flip for D3D coordinate convention.
                Graphics.Blit(m_cameraRT, m_readRT, new Vector2(1f, -1f), new Vector2(0f, 1f));
